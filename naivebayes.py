...
attributeList = []
trainData = Data()
attributes = trainData.Attribute()
...
for key in keys:
    for item in D.getAttributeValues(key):
        ...
        # })
        attributeList.append(attributes.name(key,values(len(D.selectSubset({"class":"e", key:item})),len(D.selectSubset({"class":"p", key:item}), 1/len(D.getAttributeValues(key))))))
        #print(items)
        # print(key, item)
